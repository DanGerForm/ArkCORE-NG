...
class npc_spine_of_deathwing_deathwing : public CreatureScript
{
    public:
        ...
        struct npc_spine_of_deathwing_deathwingAI : public BossAI
        {
            npc_spine_of_deathwing_deathwingAI(Creature* pCreature) : BossAI(pCreature, BOSS_SPINE)
            {
                    ...
                    switch (eventId)
                    {
                        case EVENT_END_ENCOUNTER:
                            ...
                            if (pInstance)
                            {
                                ...
                                //pInstance->NearTeleportTo(madnessdeathwingPos);
                            }                            
                            break;
                        ...
                        case EVENT_CHECK_PLAYERS:
                            if (pInstance)
                            {
                                if ((pInstance->GetBossState(BOSS_SPINE) != IN_PROGRESS) || !CheckPlayers())
                                {
                                    ResetBattle();
                                }
                                else
        ...
        private:
            ...
            void ResetBattle(bool done = false, bool despawn = true)
            {
                ...
                if (pInstance)
                {
                    pInstance->SetBossState(BOSS_SPINE, (done ? DONE : NOT_STARTED));
                    if (!done)
                    {
    ...
    public:
        ...
        struct npc_spine_of_deathwing_spawnerAI : public ScriptedAI
        {
            ...
            /*void KilledUnit(Unit* who)
            {
                ...
                {
					who->DoUpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_BE_SPELL_TARGET, 94644);
					who->DoUpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_BE_SPELL_TARGET2, 94655);
                }
            }*/
        };
};
    ...
    public:
        ...
        class spell_spine_of_deathwing_absorb_blood_SpellScript : public SpellScript
        {
            ...
            void HandleDummy(SpellEffIndex /*effIndex*/)
            {
                ...
                if (Creature* pAmalgamation = GetCaster()->ToCreature())
                {
                    Aura* aur = pAmalgamation->GetAura(SPELL_ABSORBED_BLOOD);
